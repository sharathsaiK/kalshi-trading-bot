...
void playlist_add_song(Playlist *list, int s_id, char *s_title, int s_len) {
    // TODO: Complete this function
    // TODO 3 BEGIN
    list->songs[list->count].song_id = s_id;
    list->songs[list->count].length = s_len;

    strcpy(list->songs[list->count].title, s_title);
...
Song *playlist_find_longest_song(Playlist *list) {
    ...
    if(list->count == 0) {
        ...
        Song *longest = &list->songs[0];
        for(int i = 1; i < list->count; i++) {
           if(list->songs[i].length > longest->length) {
            longest = &list->songs[i];
        }
    }  
        return longest;
    }
